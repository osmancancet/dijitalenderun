...
export default function SideAdBanner({ side }: { side: "left" | "right" }) {
  ...
  const defaultBanner = (
    ...
    >
      <div className="py-8 px-3 text-center flex flex-col items-center gap-4">
  ...
  const adBanner = ad && (
    <a href={ad.linkUrl} target="_blank" rel="noopener noreferrer" className="block hover:scale-[1.03] transition-transform">
      {ad.imageUrl ? (
        ...
        <div className="w-[140px] h-[400px] bg-gradient-to-b from-primary-dark via-primary to-primary-dark rounded-xl flex items-center justify-center shadow-lg">
