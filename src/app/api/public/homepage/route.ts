...
export async function GET() {
  try {
    ...
    const [sliderRes, gazetteRes, ilanlariRes, videolarRes, sinavRes] = await Promise.all([
      ...
      supabase.from("sinav_takvimi").select("*").eq("is_active", true).order("exam_date", { ascending: true }).limit(5),
    ]);
    ...
    return NextResponse.json(
      {
        ...
        sinavTakvimi: (sinavRes.data || []).map(toCamelCase),
      },
    ...
    return NextResponse.json(
      { error: "Veri yüklenemedi", slider: [], resmiGazete: [], personelIlanlari: [], videolar: [], sinavTakvimi: [] },
      { status: 500 }
    );
  }
}
